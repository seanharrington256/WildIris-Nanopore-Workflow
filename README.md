...
```
mkdir ~/nanopore/
cp -r /project/wy_t3_2022/example-nanopore-illumina/*fastq.gz ~/nanopore/
```
...
The porechop "check\_reads" option removes the need to specify adapters. It will automatically check and determine which ones to remove.
...
Canu is a de novo genome assembler. It does the same job for nanopore reads that Spades does for Illumina reads (or Illumina + Nanopore reads).
...
forward='paired_KphilA_CGCTCATT-AGGCGAAG_L001_R1_001.fastq.gz'
reverse='paired_KphilA_CGCTCATT-AGGCGAAG_L001_R2_001.fastq.gz'
...
I've already run Spades on just the Illumina data - compare this to the hybrid assembly and the Nanopore-only assemblies. The data are in `/project/wy_t3_2022/example-nanopore-illumina/spades_kphil_illumina` - go take a look. try counting the number of contigs of different assemblies (this should be familiar from the WGS tutorial).
...
```
DATA
PE= pe 500 50  /FULL/PATH_TO/nanopore/paired_KphilA_CGCTCATT-AGGCGAAG_L001_R1_001.fastq.gz /FULL/PATH_TO/nanopore/paired_KphilA_CGCTCATT-AGGCGAAG_L001_R2_001.fastq.gz
...
* These are example defaults, these will not always be appropriate for your data, and thinking carefully about these is necessary.
...
Genome polishing seeks to correct errors and generally improve your assembly. There are many routes you can take, and many programs to choose. We will be using three. An initial polishing of a nanopore assembly with nanopore reads with racon. Further polishing using nanopore reads with makon, and a final polishing of the assembly using illumina reads with pilon. You can do any combination of these tools, for example skip right to pilon. Try them all and compare the results!
...
Here we will polish an assembly using Illumina reads. You can do this right away, or after the racon/medaka polishing. This takes a lot of memory. We'll demonstrate this on the unpolished assembly so that we can start this up now:
...
bwa mem -t 2 canu-assembly/filt.contigs.fasta ~/nanopore/paired_KphilA_CGCTCATT-AGGCGAAG_L001_R1_001.fastq.gz ~/nanopore/paired_KphilA_CGCTCATT-AGGCGAAG_L001_R2_001.fastq.gz > mapped_to_canu.sam
...
#SBATCH --mem=0  # this will use all memory available on a node
#SBATCH --mail-type=ALL
#SBATCH --mail-user=YOUR_EMAIL@EMAIL.com
...
We probably don't need all memory of a node, but I ran out a couple times, so went all the way on the `--mem` option.
...
## Scaffolding w/ LINKS
Note that this process also requires a ton of memory. You may need to edit the amount of memory, use a reduced set of reads, or run on a high memory node on a different system if this doesn't run.
...
#SBATCH --nodes=1
#SBATCH --cpus-per-task=1
#SBATCH --mem=0 # Use all memory on a node again
#SBATCH --mail-type=ALL
#SBATCH --mail-user=YOUR_EMAIL@EMAIL.com
...
We probably don't need all memory of a node, but I ran out a couple times, so went all the way.


## Assembly Assessment

Assembly assessment can be performed using the same tools that we used for Illumina WGS assessment [here](https://github.com/seanharrington256/WY-T3-WGS-Tutorial#genome-assessment).
